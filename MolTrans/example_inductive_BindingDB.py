...
def main(fold_n, lr):
    ...
    #df_all = pd.read_csv(dataFolder + '/moltrans_AI_Bind_data.csv')
    ...
    unique_smiles = set(list(df_all['SMILES'].tolist()))
    unique_seq = set(list(df_all['Target Sequence'].tolist()))

    all_smiles['SMILES'] = list(unique_smiles)
    all_seq['Target Sequence'] = list(unique_seq)
    
    smiles_train, smiles_test_val = train_test_split(all_smiles, test_size=0.15) #Same as AI-Bind
    smiles_test, smiles_val = train_test_split(smiles_test_val, test_size=0.5) #Same as AI-Bind

    seq_train, seq_test_val = train_test_split(all_seq, test_size=0.85) #(len(df_val)+len(df_test))/len(df_all))
    seq_test, seq_val = train_test_split(seq_test_val, test_size=0.5) #len(df_test)/(len(df_val)+l
    
    df_train = df_all[df_all['SMILES'].isin(smiles_train['SMILES'].tolist()) & df_all['Target Sequence'].isin(seq_train['Target Sequence'].tolist())] 
    df_val = df_all[df_all['SMILES'].isin(smiles_val['SMILES'].tolist()) & df_all['Target Sequence'].isin(seq_val['Target Sequence'].tolist())]
    df_test = df_all[df_all['SMILES'].isin(smiles_test['SMILES'].tolist()) & df_all['Target Sequence'].isin(seq_test['Target Sequence'].tolist())]

    print('Size of train, validaiton, and test datasets: ', len(df_train), len(df_val), len(df_test))

    #import pdb; pdb.set_trace()

    #df_train, df_val_test = train_test_split(df_all, test_size=0.6) #(len(df_val)+len(df_test))/len(df_all))
    #df_val, df_test = train_test_split(df_val_test, test_size=0.8) #len(df_test)/(len(df_val)+len(df_test)))
    ...
    df_test_transductive = df_train #df_test[df_test['SMILES'].isin(train_val_smiles) & df_test['Target Sequence'].isin(train_val_aas)]
    ...
    df_test_semi_inductive = df_val #df_test[~df_test['SMILES'].isin(train_val_smiles) | ~df_test['Target Sequence'].isin(train_val_aas)]
    ...
    df_test_inductive = df_test #df_test[~df_test['SMILES'].isin(train_val_smiles) & ~df_test['Target Sequence'].isin(train_val_aas)]
